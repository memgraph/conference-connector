export const drawCurvedLine = (context, edge) => {
 ...
 */
export const getCurvedArrowShape = (edge) => {
    var _a, _b;
 ...
 */
const getPointBrezier = (edge, percentage, viaNode) => {
    const sourcePoint = edge.startNode.getCenter();
    const targetPoint = edge.endNode.getCenter();
    if (!sourcePoint || !targetPoint) {
        return { x: 0, y: 0 };
...
const findBorderPoint = (edge, nearNode) => {
    ...
    while (low <= high && iteration < maxIterations) {
        middle = (low + high) * 0.5;
        pos = Object.assign(Object.assign({}, getPointBrezier(edge, middle, viaNode)), { t: 0 });
...
//# sourceMappingURL=edge-curved.js.map